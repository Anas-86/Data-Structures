...
public class AvlTree implements Tree {
    ...
    private Node delete(Node node, int data) {
        ...
        if (data < node.getData()) {  // data smaller than given node's data -> go to the left recursively
            ...
            Node tempNode = getPredecessor(node);        //in AVL tree you can either delete the predecessor or the successor.
    ...
    private Node settleDeletion(Node node) {
        ...
        return node;        //in this case the balance is one of either three cases: 1,0,-1
    }

    private Node getPredecessor(Node node) {


        Node predecessor = node.getLeftNode();
