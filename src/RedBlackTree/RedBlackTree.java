	...
	private void fixViolations(Node node) {
		...
		while( node != root && node.getColor() != NodeColor.BLACK && node.getParent().getColor() == NodeColor.RED ) {
			...
			if( parentNode == grandParentNode.getLeftChild() ) {
				...
				if( uncle != null && uncle.getColor() == NodeColor.RED ) {
				...
				} else {		//the uncle is either null or its color is black
					...
					//if the inserted node is a left child
			...
			} else {		//if the parent node is the right child.
				...
				if( uncle != null && uncle.getColor() == NodeColor.RED ) {
					...
					//the inserted node is a right child
		...
		//the inserted node is either the root or its color is black or the color of its parent is black.
		//if the inserted node is the root, recolor it black, otherwise, do nothing.
